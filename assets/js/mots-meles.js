...
function createLevelCard(levelId, level, progress) {
  ...
  if (levelId === 1) {
    ...
    if (
      (progress[prevId] && progress[prevId].completed) ||
  ...
  const isCompleted =
    (progress[levelId] && progress[levelId].completed) || false;
  ...
  let lockMsg = !isUnlocked
    ? `<div class="lock-message"><i class="fa-solid fa-lock"></i> Bloqué</div>`
    : "";
...
function filterLevels(difficulty) {
  ...
  document
    .querySelectorAll(".difficulty-badge")
  ...
  document.querySelectorAll(".level-card").forEach((card) => {
    card.style.display =
      card.getAttribute("data-difficulty") === difficulty ? "block" : "none";
  });
}
...
function updateProgressStats() {
  ...
  if (document.getElementById("easyProgress"))
    document.getElementById("easyProgress").textContent = `${c.easy}/6`;
  if (document.getElementById("mediumProgress"))
    document.getElementById("mediumProgress").textContent = `${c.medium}/6`;
  if (document.getElementById("hardProgress"))
    document.getElementById("hardProgress").textContent = `${c.hard}/6`;
}
  ...
  document.querySelectorAll(".level-card").forEach((card) => {
    ...
    if (levelId && difficulty) {
      ...
      if (bestScore) {
        ...
        badge.innerHTML = `<div class="score-badge-content"><div class="score-points"><i class="fa-solid fa-star"></i> ${
          bestScore.score
        } pts</div><div class="score-time"><i class="fa-solid fa-clock"></i> ${formatTime(
          bestScore.time
        )}</div></div>`;
      }
    }
...
function updateLevelStars(card, gameType, difficulty, levelId) {
  ...
  let html = "";
  for (let i = 0; i < 3; i++)
    html += `<i class="${i < stars ? "fa-solid" : "fa-regular"} fa-star star ${
...
function displayGlobalStats(gameType) {
  ...
  if (!gameStats && allScores["mots-meles"])
    gameStats = allScores["mots-meles"];
  if (!gameStats && allScores["mots_meles"])
    gameStats = allScores["mots_meles"];
