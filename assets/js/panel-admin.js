  ...
  fetch("api/get-stats.php")
    ...
    .then((data) => {
      if (data.success) {
        ...
        ).textContent = formatNumber(stats.total_users);
        document.querySelector(
          ".stat-card:nth-child(2) .stat-value"
        ).textContent = formatNumber(stats.total_parties);
        document.querySelector(
          ".stat-card:nth-child(3) .stat-value"
        ).textContent = formatNumber(stats.total_score);
        document.querySelector(
          ".stat-card:nth-child(4) .stat-value"
        ).textContent = stats.croissance;
      }
    })
    .catch((error) => {
...
function updateUsersTable(users) {
  ...
  users.forEach((user) => {
    ...
    row.innerHTML = `
            ...
            <td>
                <select class="role-select" onchange="changeRole(${
                  user.id
                }, this.value, this)" data-current="${user.role}">
                    <option value="joueur" ${
                      user.role === "joueur" ? "selected" : ""
                    }>👤 Joueur</option>
                    <option value="admin" ${
            ...
            <td><button class="action-btn delete-btn" onclick="deleteUser(${
              user.id
            }, '${user.pseudo}')">Supprimer</button></td>
        `;
...
function updateScoresTable(users) {
  ...
  sortedUsers.forEach((user, index) => {
    ...
    row.innerHTML = `
            ...
            <td><span class="score-badge">${formatNumber(
              user.score_total
            )}</span></td>
...
function changeRole(userId, newRole, selectElement) {
  ...
  const roleDisplayName =
    newRole === "admin" ? "👑 Administrateur" : "👤 Joueur";
  const pseudo = selectElement
    .closest("tr")
    .querySelector(".user-info span").textContent;
  ...
  fetch("api/change-role.php", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        if (pendingRoleChange.selectElement) {
          pendingRoleChange.selectElement.dataset.current =
